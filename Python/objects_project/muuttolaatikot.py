...
Simple class and user interface for tracking moving boxes.
"""
...
class MovingBox:
    ...
    def __init__(self, box_name):
        """
        Constructor. Initializes a box and a dictionary which will contain
    ...
    def add_item(self, item_name, item_count):
        """
        Adds items to <contents>. Assumes that <item_name> and <item_count>
        are in correct format.

        :param item_name: str, name of the item to be added to the box.
        :param item_count: int, the number of items to add.
        """
        # If the item didn't exist in the box before, create new slot.
        if item_name not in self.__contents:
            self.__contents[item_name] = item_count

        # Otherwise add the number of items to the existing count.
        else:
            self.__contents[item_name] += item_count

    def remove_item(self, item_name, item_count):
        """
        Removes items from <contents>. Assumes that <item_name> and
        <item_count> are in correct format. Returns False if trying to remove
        object that doesn't exist in <contents> or if <item_count> is larger
        than the number of objects. Returns True if removal was successful.

        :param item_name: str, name of the to be removed from the box.
        :param item_count: int, the number of items to remove.
        :return: False if removal failed, True if successful
        """
        # Checks that the item is found in the box.
        if item_name not in self.__contents.keys():
            return False

        # And that we don't remove more items than we have.
        elif self.__contents[item_name] < item_count:
            return False

        # Removes the items. If 0 remain, deletes the item from the box.
        else:
            self.__contents[item_name] -= item_count
            if self.__contents[item_name] == 0:
                self.__contents.pop(item_name)

            return True

    def transfer_item(self, target_box, item_name, item_count):
        """
        Transfers <item_count> number of objects named <item_name> from
        "self"-box to <target_box>. Assumes all information is in correct
        format. Returns False if the items can't be transferred. Returns True
        if transfer was successful.

        :param target_box: str, name of the box into which the items will be
        moved.
        :param item_name: str, name of the item to be transferred.
        :param item_count: int, number of items to transfer.
        :return: False if transfer failed, True if successful
        """
        # Utilize the remove_item -function to remove items and to check that
        # the items can be removed.
        if not self.remove_item(item_name, item_count):
            return False

        # If the removal was successful add the items to the new box.
        target_box.add_item(item_name, item_count)
        return True

    def list_content(self):
        """
        Prints the contents of the box and the total number of items in the
        box.
        """
        # Variable for the total number of items in the box.
        amount = 0

        # Count the total number of items in the box.
        for item in self.__contents:
            amount += self.__contents[item]

        # Prints the header.
        print(f"Box \"{self.__name}\" contains {amount} items.")

        # Prints all contents in alphabetical order.
        for item in sorted(self.__contents):
            print(f"{self.__contents[item]:3} {item}")

    def search_item(self, item_name):
        """
        Checks how many <item_name>s are in the box. If there are none,
        returns 0.

        :param item_name: str, the item looked for.
        :return: int, the number of <item_name>s in the box, 0 if none are
        found.
        """
        # Variable for the total number of given item in the box.
        amount = 0

        # If <item_name> is in the box, update the value
        if item_name in self.__contents:
            amount = self.__contents[item_name]
