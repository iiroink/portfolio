...
Sähköposti: iiro.inkinen@tuni.fi
"""
...
def read_friendship_network(filename):
    ...
    # This dictionary will store peoples names as keys and all of their friends
    # as a list in the payload.
    network = {}
...
def add_friendship(network, name1, name2):
    ...
    for name in [ name1, name2 ]:
        if not name.isalpha():
            print(
                f"Error: '{name}' is not a valid name: friendship not added.")
            return
    ...
    # If <name1> isn't in the <network> dict, add them and an empty list for
    # upcoming friends.
    if name1 not in network:
        network[name1] = []

    # Same operation for name2 since friendships are two-way.
    if name2 not in network:
        network[name2] = []

    # Check if <name1> and <name2> are already friends.
    if name1 in network[name2]:
        return

    # Add <name2> as a friend of <name1> and vice versa.
...
def print_command(network, namelist):
    ...
    # Print all names in the dict in alphabetical order.
    for person in names:
        print(person)

        # Nested loop prints all their friends in alphabetical order.
...
def friends_command(network, namelist):
    ...
    # Checks that <name> is found in the network dict as a key.
...
def common_friends_command(network, namelist):
    ...
    # Checks that the names are in the network dict.
    for name in namelist:
        if name not in network:
            print(f"Error: '{name}' is an unknown name.")
            return
    ...
    # A person can't have common friends with themselves.
    if name1 == name2:
        print(f"{name1} has no common friends with him/herself.")
        return

    # Initialize a list that will have the common friends.
    common_friends = []

    # Checks if a friend of <name1> is also a friend of <name2>. If so adds it
    # to <common_friends>.
    for friend in network[name1]:
        if friend in network[name2]:
            common_friends.append(friend)

    # If there are no common friends print an error message.
    if len(common_friends) == 0:
        print(f"{name1} and {name2} have no common friends.")
        return

    # Print common friends in alphabetical order.
