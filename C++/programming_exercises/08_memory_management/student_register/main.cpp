...
bool update_phone_number(Student& s) {
    std::cout << "Enter a new phone number: ";
    std::string new_number = "0";
...
//        information = split(line, ';', true);
//        if (information[0] == s.name) {
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            std::string student_n = parts.at(1);
            if (student_numbers.find(student_n) == student_numbers.end()) {
                std::cout << "There is no student with the given number!"
                          << std::endl << std::endl;
                continue;
            } else {
                if (update_phone_number(*student_numbers.at(student_n))) {
                    std::ofstream file(file_name);
                    for (auto& student : user_ids) {
                        file << student.second->student_number << ";"
