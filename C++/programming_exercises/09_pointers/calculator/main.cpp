...
int main() {
    ...
    while ( true ) {
        ...
        double right = 0;
        if (is_command) {
            if (execute.parameter_number != pieces.size()-1) {
                cout << "Error: wrong number of parameters." << endl;
            } else if (execute.is_exit) {
                cout << GREETING_AT_END << endl;
                return EXIT_SUCCESS;
            } else if (! string_to_double(pieces.at(1), left) or ! string_to_double(pieces.at(2), right)) {
                cout << "Error: a non-number operand." << endl;
            } else {
                double result = execute.action(left, right);
                cout << result << endl;
            }
        } else {
