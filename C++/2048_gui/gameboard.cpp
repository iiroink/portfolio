...
void GameBoard::fill(int seed)
{
    ...
    for( auto y = 0; y < SIZE; ++y )
    {
        ...
        {
            // Only init NumberTiles if the board is empty.
            if (board_.at(y).at(x) == nullptr) {
                board_.at(y).at(x) = new NumberTile(0, std::make_pair(y, x),
                                                    this);
            // Otherwise just set all numbers to 0.
            } else {
                board_.at(y).at(x)->set_zero();
            }
        }
    }
...
bool GameBoard::move(Coords dir, int goal, int& score)
{
    bool has_won = false;
    for( unsigned y = 0; y < board_.size(); ++y )
    {
        for( unsigned x = 0; x < board_.front().size(); ++x )
        {
            ...
            if( board_.at(directed_y).at(directed_x)->move(dir, goal, score) )
            {
                has_won = true;
            }
        }
